            ...
            <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                <div className="grid grid-cols-7 mb-2 text-center">
                    {[0, 1, 2, 3, 4, 5, 6].map((day) => (
                        <div
                            key={day}
                            className={`py-2 font-medium ${getDayColor(day)}`}
                        >
                            {getDayName(day)}
                        </div>
                    ))}
                ...
                <div className="grid grid-cols-7 gap-2 mb-8">
                    {calendarData.flat().map((day, index) => (
                        <div key={index} className="relative">
                            ...
                            >
                                {(!expandedDate ||
                                    !isSameDate(day.date, expandedDate)) && (
                                    <div
                                        className={`h-36 p-2 rounded-xl overflow-hidden transition-all
                ${
                    day.currentMonth
                        ? "bg-white/10 hover:bg-white/15"
                        : "bg-white/5 text-gray-500"
                }
                ${isToday(day.date) ? "ring-2 ring-blue-500/50" : ""}
